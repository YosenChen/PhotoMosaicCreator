...
public:
	...
	char *mSubImgFileDir, *mProfImgFileName, *mOutImgFileName;
	int mSubImgCounter;
	bool mHasNoMoreFile;
	...
	/*this function can be indep used with given image dir*/void subImages_SaveAs_norTempJPGImage_inCreatedTempDir(char *subImgOrigDir, char *tempDirName, int subImgLongLen, int subImgShortLen);

	INIT_IO_RETURN initIOBasis(IMG_CONF_MODE conf_mode, CvSize subImgSize, CvSize subGridSize, CvSize outImgSize, 
					 char* subImgFileDir, char* profileImgFileName, char* outImgFileName);
	...
	/*this function can be indep used with given img*/void setCurSubImg(int gridX, int gridY, IplImage* loadImgPtr, char *loadImgName, bool isManualMode = false); //will be resized to subImgSize
	bool fillCurSubImg();

	/*this function can be indep used with given img*/bool imageMixing(IplImage* inImg1, IplImage* inImg2, float alpha1, IplImage* outImg, bool isManualMode = false);

	void show_and_save_OutImg();

	/*this function can be indep used with given img*/void addText(char* loadImgName_NoText, char* saveImgName_Text, 
